...
public record ThreadState(String actorName, int checkpoint, int loopCount, List<Integer> ownedMonitors, List<Integer> ownedLocks,
    ...
    public Collection<ThreadState> dependencies(RuntimeState state) {
        var monitor = waitingForMonitor.filter(monId -> !state.ownedMonitors().containsKey(monId)).map(mon -> state.ownedMonitors().get(mon)).stream();
        var locks = waitingForLock.filter(monId -> !state.lockedLocks().containsKey(monId)).map(mon -> state.lockedLocks().get(mon)).stream();
